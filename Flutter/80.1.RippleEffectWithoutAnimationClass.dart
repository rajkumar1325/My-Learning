...
class MyHomeState extends State<MyHomePage> with SingleTickerProviderStateMixin{  //adding another class using 'with'
  ...
  void initState() {
    ...
    _animationController = AnimationController(vsync: this, duration: Duration(seconds: 4), lowerBound: 0.5); //'this' --> references to SingleTickerProviderStateMixin
    // _animation = Tween(begin:0.0, end:1.0).animate(_animationController); //start and end state/size

    _animationController.addListener( (){ ///listener records from 0 to 200 during an interval of 4sec
          ...
          children: radiusList.map((e){
            return Container(
              ...
              decoration: BoxDecoration(
                ...
                color: Colors.blue.withValues(alpha: 1.0 - _animationController.value) //opacity down as we move forward
              ),
            );
          } ).toList(),
        ),
